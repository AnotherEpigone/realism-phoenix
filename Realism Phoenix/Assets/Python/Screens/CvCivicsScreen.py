...
class CvCivicsScreen:
	...
	def __init__(self):
		...
		self.HEADINGS_TOP = 45 # was 70
		self.HEADINGS_SPACING = 5
		self.HEADINGS_BOTTOM = 270 # was 280
		self.HELP_TOP = 340 # was 350
	...
	def interfaceScreen (self):
		...
		# Reset a bunch of Y coordinates if there's room on the screen to stretch things out a bit
		self.H_SCREEN = screen.getYResolution() - 250
		self.HELP_BOTTOM = self.H_SCREEN - 118
		self.BOTTOM_LINE_TOP = self.H_SCREEN - 108
		self.Y_EXIT = self.H_SCREEN - 42
		self.Y_CANCEL = self.H_SCREEN - 42
		self.Y_BOTTOM_BAR = self.H_SCREEN - 55
	
		screen.setRenderInterfaceOnly(True)
		screen.showScreen(PopupStates.POPUPSTATE_IMMEDIATE, False)
	
		# Set the background and exit button, and show the screen
		screen.setDimensions(screen.centerX(0), self.Y_SCREEN, self.W_SCREEN, self.H_SCREEN)
		screen.addDDSGFC(self.BACKGROUND_ID, ArtFileMgr.getInterfaceArtInfo("MAINMENU_SLIDESHOW_LOAD").getPath(), 0, 0, self.W_SCREEN, self.H_SCREEN, WidgetTypes.WIDGET_GENERAL, -1, -1 )
		screen.addPanel( "TechTopPanel", u"", u"", True, False, 0, 0, self.W_SCREEN, 55, PanelStyles.PANEL_STYLE_TOPBAR )
		screen.addPanel( "TechBottomPanel", u"", u"", True, False, 0, self.Y_BOTTOM_BAR, self.W_SCREEN, 55, PanelStyles.PANEL_STYLE_BOTTOMBAR )
...
## Revolutions ##
		...
		screen.addMultilineText(szHelpAreaID, szHelpText, fX+5, fY, self.HEADINGS_WIDTH-7, self.HELP_BOTTOM - fY-15, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)				
	...
	def updateAnarchy(self):
		...
		screen.setLabel("CivicsUpkeepText", "Background", u"<font=3>" + szText + u"</font>", CvUtil.FONT_CENTER_JUSTIFY, self.X_SCREEN, self.BOTTOM_LINE_TOP + self.BOTTOM_LINE_HEIGHT - 3 * self.TEXT_MARGIN, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1)
